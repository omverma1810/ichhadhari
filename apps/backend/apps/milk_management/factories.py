...
class MilkCollectionFactory(DjangoModelFactory):
    ...
    fat_percentage = factory.Faker('pydecimal', left_digits=1, right_digits=2, positive=True, min_value=3, max_value=9)
    snf_percentage = factory.Faker('pydecimal', left_digits=1, right_digits=2, positive=True, min_value=7, max_value=10)
    temperature = factory.Faker('pydecimal', left_digits=1, right_digits=1, positive=True, min_value=2, max_value=8)
    rate_per_liter = Decimal('45.00')
